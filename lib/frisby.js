...
function Frisby(msg, hInstance) {
  ...
  // set the handle to the instance
...
Frisby.prototype.promisedToss = function(retry) {
  var self = this;
  var hInstance = self.hInstance;
  ...
  if (hInstance && hInstance.COOKIE) {
    self.addHeader('Cookie', hInstance.COOKIE);
  }

  this.after(function(req, res, body) {
    if (hInstance && res.headers['set-cookie']) {
      ...
      it("\n\t[ " + self.current.itInfo + " ]", function(done) {
        ...
        runs(function() {
          ...
          resolve({
            ...
            text: self.current.response.body
          });
          //done();
          fnDone = done;
        })
...
exports.newInstance = function() {
  return new FrisbyInstance();
}

function FrisbyInstance() {
  var self = this;

  this.COOKIE = '';

  self.create = function(msg) {
    return new Frisby(msg, self);
  };
}
