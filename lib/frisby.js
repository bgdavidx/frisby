...
Frisby.prototype._request = function (/* method [uri, data, params] */) {
  ...
  if (hInstance.COOKIE) {
    outgoing.headers['cookie'] = hInstance.COOKIE.join('; ')
  }
...
Frisby.prototype.promisedToss = function(retry, delay) {
  var self = this;
  if (typeof retry === "undefined") {
    retry = self.current.retry;
  }
  ...
  if (hInstance && hInstance.COOKIE) {
    self.addHeader('Cookie', hInstance.COOKIE.join('; '));
  }

  this.after(function(req, res, body) {
    if (res.headers['set-cookie']) {
      var setCookie = res.headers['set-cookie'];
      var cookie = [ setCookie ]

      if (Array.isArray(setCookie)) {
        cookie = setCookie.map(function(x) {
          return x.split(';')[0];
        })
      }

      if (hInstance) {
        hInstance.COOKIE = _.chain(cookie).concat(hInstance.COOKIE || []).uniq(function(item) {
          return item.split('=')[0]
        }).value()
      }
    }
  ...
  var promise; // promise to resolve
  var fnDone; //
  ...
  return new Promise(function(resolve, reject) {
      ...
      it("\n\t[ " + self.current.itInfo + " ]", function(done) {
        ...
        var mainFunc = function() {
          ...
          runs(function() {
            ...
            resolve({
              content: self.current.response.body,
              json: _jsonParse(self.current.response.body)
            });
            //done();
            fnDone = done;
          })
    ...
    return delay ? Promise.delay(delay).return(stuff) : stuff;
  }).finally(function() {
    fnDone();
  });
};
...
function FInstance() {
  ...
  this.create = function(msg) {
    return new Frisby(msg, this);
  }
}

exports.newInstance = function() {
  return new FInstance();
};
